...
//------------------------------------------------
void ofxBox2dCircle::setup(b2World * b2dworld, float x, float y, float radius, float lDamping, float aDamping) {
	...
	bodyDef.linearDamping = lDamping;
	bodyDef.angularDamping = aDamping;
	//bodyDef.gravityScale = 0.0f;

	body  = b2dworld->CreateBody(&bodyDef);
	body->CreateFixture(&fixture);
}
void ofxBox2dCircle::setup(b2World * b2dworld, ofVec2f &pts, float radius, float lDamping, float aDamping) {
    setup(b2dworld, pts.x, pts.y, radius, lDamping, aDamping);
}
...
void ofxBox2dCircle::addImpulseForce(ofVec2f pt, ofVec2f amt) {
	if(body != NULL) {
...
void ofxBox2dCircle::increaseGravity(float amt) {
    //this->amt = amt;

    //bodyDef.gravityScale += amt;
